...
class SF_Dict:
    ...
    def _init_variational(self, smoothness):
        ...
        self.b = 1./np.random.gamma(smoothness, 1./smoothness, size=(self.T, self.L))
    ...
    def update_theta_batch(self, disp):
        def f(theta):
            a, b = theta[:self.T * self.L].reshape(self.T, self.L), np.exp(theta[-self.T * self.L:].reshape(self.T, self.L))
            EA, EA2, ElogA = comp_expect(a, b)
