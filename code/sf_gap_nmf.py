...
class SF_GaP_NMF(gap_nmf.GaP_NMF):
    ...
    def update_a(self, k):
        def f(theta):
            ...
            val -= np.sum((self.alpha - rho) * Ea)
            val += np.sum(special.gammaln(nu) - nu * np.log(rho))

            Eexpa = self.comp_exp_expect(nu, rho, self.U)
            val -= np.sum(self.gamma * self.Ew[:, k] * np.prod(Eexpa, axis=1))
            val -= np.sum(self.gamma * np.dot(self.U, Ea))
            return -val
        ...
        def df(theta):
            ...
            grad_nu = np.sum((self.Ew[:, k] * log_term * np.prod(Eexpa, axis=1)
        ...
        self.Eexpa[:, :, k] = self.comp_exp_expect(self.nua[:, k],
                                                   self.rhoa[:, k],
                                                   self.U)
    ...
    def update_w(self):
        ...
        self.rhow[:, goodk] = self.gamma * np.prod(self.Eexpa[:, :, goodk],
                                                   axis=1)
